...
function configureMarkedRenderer(marked: any): void {
  try {
    ...
    const originalLink =
      ...
      function (href: string, title: string | null, text: string): string {
    ...
    renderer.link = function (href: string, title: string | null, text: string): string {
      ...
      const filenamePattern =
        /^[^/\s\\:<>"'`]+\.(md|txt|csv|json|yml|yaml|xml|html|css|js|ts|py|rb|go|rs|php|java|cpp|c|h|hpp|sh|bat|ps1|sql|db|sqlite|log|ini|cfg|conf|lock|lockfile|pdf|doc|docx|xls|xlsx|ppt|pptx|zip|tar|gz|rar|7z|png|jpg|jpeg|gif|webp|svg|bmp|ico|mp4|mp3|avi|mov|wav|flac|woff|woff2|ttf|otf|eot)$/i;
